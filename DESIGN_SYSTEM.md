...
- 页面组件不得新增字号。
- 按钮默认使用 14px，紧凑控件使用 12px；手机端可输入的 `input`、`textarea`、`select` 统一使用 16px，避免 iOS 聚焦输入时自动放大页面，桌面端输入文字仍使用 14px。
...
- 标准高度：46px；紧凑高度：36px。
- 输入文字：桌面端 14px；手机端所有可输入控件统一 16px。辅助输入和标签编辑可在桌面端使用 12px，但手机端同样提升至 16px。
- 背景使用主表面，边框使用默认边框。
- focus 使用品牌边框与 3px 低透明度品牌色焦点环。
- textarea 最小高度按内容场景选择 80、128、192px；待发布确认框使用专用的 200px，不随意新增。
...
- 待发布确认框使用 `.ui-ready-editor`：默认高度 200px，可由用户继续向下拖动。
